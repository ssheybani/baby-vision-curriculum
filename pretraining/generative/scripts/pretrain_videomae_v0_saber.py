...
def DDP_process(rank, world_size, args, verbose=True):#protocol, seed):
    ...
    mask_ratio = float(args.mask_ratio)/100 #0.9
...
#                     break #@@@
    ...
    if is_main_process():
        results_fpath = os.path.join(model_dir,f"train_val_scores_{protocol}_seed_{seed}"+args.other_id)
        results_df.to_csv(results_fpath, sep=',', float_format='%.4f')
        # the model
        # STAGE = "g2"
        model_fname = '_'.join(['model', protocol, 'seed',str(seed), 'other', str(other_seed), args.other_id])+'.pt'
