...
# def grad_logger(named_params, op='mean'):
#     stats = AverageMeter()
...
#             if op=='mean':
#                 grad_norm = float(torch.norm(p.grad.data.mean(dim=0)))
#             elif op=='std':
#                 grad_norm = float(torch.norm(p.grad.data.std(dim=0)))
