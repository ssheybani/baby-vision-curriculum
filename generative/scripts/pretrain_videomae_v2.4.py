...
# added in v2.4:
# make_dataset: better fold definition: every 3rd 30min video segments.
# gradient monitoring
...
def append_gradient_statistics(
    ...
    for paramgr_name, paramgr_idx in zip(paramgr_names, paramgr_indices):
        ...
        tmp_cgmean = np.abs(cgrads.mean(axis=0))
        tmp_cgstd = cgrads.std(axis=0)
        tmp_cgsnr = np.log10((tmp_cgmean+1e-12)/(tmp_cgstd+1e-12))
